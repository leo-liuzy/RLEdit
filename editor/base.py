...
class BaseEditor:
    ...
    def train_sequence(self, loader: DataLoader, save=False):
    ...
    def train_new(self, loader: DataLoader, save=False):
        ...
        for _, tuples in enumerate(tqdm(limited_loader, desc="Train", ncols=100, total=max_steps)):
            ...
            try:
                ...
                save_file(self.model.state_dict(), "/root/autodl-tmp/modified_model/model.safetensors") # 保存现在的模型
                self.reset_model() # 读取初始模型
                self.cache(merged_tuples) # 使用初始模型获得微调梯度
                param_shifts_now = self.predict_param_shifts() # 使用初始模型预测参数变化
                config = AutoConfig.from_pretrained("/root/autodl-tmp/modified_model/")
                state_dict = load_file("/root/autodl-tmp/modified_model/model.safetensors")
    ...
    def valid_test(self, loader: DataLoader):
    ...
    def sequence_edit_valid_last(self, loader: DataLoader):
    ...
    # def sequential_valid(self, tuples: DataLoader):
    ...
    def cache(self, tuples: List[Dict[str, torch.LongTensor]]):
        ...
        for idx, t in enumerate(tuples):
            ...
            for module_idx, module_name in enumerate(self.config.model.edit_modules):
                ...
                # keys = tr[module_name].keys.to(torch.float32)
                # values_grad = tr[module_name].values_grad.to(torch.float32)
    ...
    #     empty_cache(self.config.editor.cache_dir, self.config)
    #     for _ in range(self.config.editor.n_epochs):
    #         for tuples in tqdm(valid_loader, desc = "Valid", ncols = 100):
    #             self.train(train_loader)
    #             self.sequential_valid(tuples)
    #             empty_cache(self.config.editor.cache_dir, self.config)
    #             self.reset_hypernet()


    # def run_sequential_all(self, train_loader: DataLoader, valid_loader: DataLoader):
    #     empty_cache(self.config.editor.cache_dir, self.config)
    #     for _ in range(self.config.editor.n_epochs):
    #         # for tuples in tqdm(valid_loader, desc = "Valid", ncols = 100):
    #             # tuple_dict["edit_tuples"].extend(tuples["edit_tuples"])
    #             # tuple_dict["equiv_tuples"].extend(tuples["equiv_tuples"])
    #             # tuple_dict["unrel_tuples"].extend(tuples["unrel_tuples"])
    #         self.train_sequence(train_loader)
    #         self.model = self.ori_model
    #         tuple_dict = {"edit_tuples": [], "equiv_tuples": [], "unrel_tuples": []}
    #         for tuples in tqdm(valid_loader, desc = "Valid", ncols = 100):
    #             tuple_dict["edit_tuples"].extend(tuples["edit_tuples"])
    #             tuple_dict["equiv_tuples"].extend(tuples["equiv_tuples"])
    #             tuple_dict["unrel_tuples"].extend(tuples["unrel_tuples"])
    #             self.sequential_valid(tuple_dict)
    #         empty_cache(self.config.editor.cache_dir, self.config)
    #         self.reset_hypernet()


    def run_sequential_all_test(self, train_loader: DataLoader, valid_loader: DataLoader):
        ...
        for _ in tqdm(range(self.config.editor.n_epochs), desc = "epoch"):
            ...
            self.reset_model()
            self.train_sequence(train_loader)
            # self.train_new(train_loader)
            self.reset_model()
            if self.config.editor.save_checkpoint:
                torch.save(self.net.state_dict(), f"checkpoints/{self.config.model.name}_{self.config.editor.name}_{str(self.config.data.n_edits)}_net.pth")
                torch.save(self.opt.state_dict(), f"checkpoints/{self.config.model.name}_{self.config.editor.name}_{str(self.config.data.n_edits)}_opt.pth")
                print("-----Saved checkpoints-----")
            self.sequence_edit_valid_last(valid_loader)
            # self.valid_test(valid_loader)
            empty_cache(self.config.editor.cache_dir, self.config)
    ...
    def run_retrain(self, train_loader: DataLoader, valid_loader: DataLoader):
        ...
        for num in range(10):
            ...
            # 保存LM
            save_directory = "/root/autodl-tmp/retrain_llama-3"
            self.model.save_pretrained(save_directory, safe_serialization=True)
            print("Successfully save edited-model!")
            # tokenizer.save_pretrained(save_directory)  # 保存分词器（可选）
            ...
            del self.model
            torch.cuda.empty_cache()
            self.model = make_model_retrain(self.config.model).to(self.config.model_device)
            for _ in range(5):
                self.valid_test(valid_loader)
    ...
    def glue_eval(self, num_of_tests):
        tokenizer = AutoTokenizer.from_pretrained(self.config.model.name_or_path)
        glue_eval = GLUEEval(
            self.model,
            tokenizer,
